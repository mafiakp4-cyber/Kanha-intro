...
async def set_intro(client: Client, message: Message):
    ...
    try:
        ...
        # Absolute path use kar ke save karo
        intro_file_path = os.path.join(os.getcwd(), f"intro_{message.from_user.id}.mp4")
        file_path = await client.download_media(message.reply_to_message.video.file_id, file_name=intro_file_path)
        user_intros[message.from_user.id] = file_path
